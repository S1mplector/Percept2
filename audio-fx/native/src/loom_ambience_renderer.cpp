...
void LoomAmbienceRenderer::configure(const std::string& preset, float intensity, float volume, bool loop) {
...
int LoomAmbienceRenderer::render(uint8_t* pcm, int frames) {
  ...
  for (int i = 0; i < frames; ++i) {
    ...
    const float pan = 0.18f * panLfo_.sine();
    panLfo_.advance();
...
void LoomAmbienceRenderer::updateFilters() {
  ...
  lowPassLow_.setCoefficients(BiquadFilter::Type::LowPass, 200.0f * intensityMod, 0.7f, sr);
  lowPassMid_.setCoefficients(BiquadFilter::Type::LowPass, 800.0f * intensityMod, 0.5f, sr);
  highPassMid_.setCoefficients(BiquadFilter::Type::HighPass, 150.0f, 0.5f, sr);
  highPassHigh_.setCoefficients(BiquadFilter::Type::HighPass, 2000.0f + intensity_ * 1500.0f, 0.6f, sr);
  lowPassHigh_.setCoefficients(BiquadFilter::Type::LowPass, 8000.0f, 0.45f, sr);
  gustFilter_.setCoefficients(BiquadFilter::Type::BandPass, 400.0f, 1.5f, sr);

  rainBedLowPass_.setCoefficients(BiquadFilter::Type::LowPass, 6500.0f + intensity_ * 4000.0f, 0.55f, sr);
  rainBedHighPass_.setCoefficients(BiquadFilter::Type::HighPass, 350.0f, 0.55f, sr);
  rainHissHighPass_.setCoefficients(BiquadFilter::Type::HighPass, 4500.0f - intensity_ * 500.0f, 0.7f, sr);
  rainDropBandPass_.setCoefficients(BiquadFilter::Type::BandPass, 1800.0f + intensity_ * 1200.0f, 1.2f, sr);

  oceanSwellLowPass_.setCoefficients(BiquadFilter::Type::LowPass, 220.0f + intensity_ * 120.0f, 0.8f, sr);
  oceanWashBandPass_.setCoefficients(BiquadFilter::Type::BandPass, 700.0f + intensity_ * 500.0f, 0.7f, sr);
  oceanFoamHighPass_.setCoefficients(BiquadFilter::Type::HighPass, 2800.0f, 0.7f, sr);
}

void LoomAmbienceRenderer::maybeTriggerWindGust(float dt) {
  if (gust_.isActive()) return;
  gustTimer_ += dt;
  const float gustChance = 0.08f * (0.35f + intensity_ * 1.15f);
  if (gustTimer_ > 1.0f / std::max(0.05f, gustChance)) {
    gust_.trigger(0.35f + intensity_ * 0.75f);
    gustTimer_ = 0.0f;
  }
}
...
float LoomAmbienceRenderer::synthesizeWindSample() {
  ...
  const float midLayer = midNoise * 0.6f * shapedIntensity * medMod * (1.0f + shapedIntensity * 0.3f) * 0.85f;

  float highNoise = highPassHigh_.process(noiseHigh_.pink());
  highNoise = lowPassHigh_.process(highNoise);
  const float highLayer = highNoise * 0.4f * std::pow(shapedIntensity, 3.0f) * fastMod * 0.25f;

  float gustNoise = gustFilter_.process(noiseGust_.pink());
  const float gustLayer = gustNoise * gustEnvelope * 0.55f;

  float speedWhoosh = 0.0f;
  if (intensity_ > 0.35f) {
    const float whooshNoise = noiseMid_.filtered(0.08f + intensity_ * 0.2f, 0.3f);
...
float LoomAmbienceRenderer::synthesizeRainSample() {
  ...
  mediumLfo_.advance();
  const float slowMod = slowLfo_.sine() * 0.5f + 0.5f;
  const float density = 0.25f + intensity_ * 0.75f;

  float bed = rainBedLowPass_.process(noiseMid_.pink());
  bed = rainBedHighPass_.process(bed);
  bed *= 0.24f + density * 0.42f;

  float hiss = rainHissHighPass_.process(noiseHigh_.white());
  hiss *= 0.06f + intensity_ * 0.18f;

  const float triggerThreshold = 0.985f - intensity_ * 0.09f;
  if (noiseDrop_.white() > triggerThreshold) {
    dropletEnvelope_ = 0.35f + intensity_ * 0.55f;
  }
  dropletEnvelope_ *= 0.9925f - intensity_ * 0.0015f;
...
float LoomAmbienceRenderer::synthesizeOceanSample() {
  ...
  slowLfo_.advance();
  mediumLfo_.advance();
  const float waveCycle = 0.5f * (1.0f + std::sin(elapsedSeconds_ * (0.11f + intensity_ * 0.07f) * 2.0f * kPi));

  float swell = oceanSwellLowPass_.process(noiseLow_.brown());
  swell *= 0.18f + intensity_ * 0.20f;

  float wash = oceanWashBandPass_.process(noiseMid_.pink());
  wash *= (0.12f + intensity_ * 0.28f) * (0.35f + 0.65f * waveCycle) * (0.85f + medMod * 0.15f);

  float foam = oceanFoamHighPass_.process(noiseHigh_.white());
  const float crest = std::max(0.0f, (waveCycle - 0.58f) / 0.42f);
  foam *= crest * crest * (0.08f + intensity_ * 0.18f) * (0.8f + slowMod * 0.2f);
...
float LoomAmbienceRenderer::nextMonoSample() {
  ...
  return sample * volume_ * (0.20f + 0.80f * intensity_);
}
