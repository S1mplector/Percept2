...
The Inspector panel shows editable properties for whatever entity is selected in the JES visual editor canvas. It dynamically rebuilds its grouped UI when the selection changes, showing only the fields relevant to the selected entity's type.
...
<!-- AUTO-INSPECTOR-SCREENSHOTS:START -->
### Visual Reference (Auto-Generated)

_Generated by `./gradlew :editor:generateDocsScreenshots -Djvn.docs.profile=inspector` on 2026-08-18 17:11._
...
<!-- AUTO-INSPECTOR-SCREENSHOTS:END -->
...
|-------|------|-------------|
| **Selection summary** | Read-only | Shows a friendly entity type (for example, "Sprite") |
...
| **Scale X / Y** | Number | Horizontal and vertical scale; negative values flip the entity |
| **Origin X / Y** | Number | Transform pivot coordinates |
| **Depth (Z)** | Number | Draw order; larger values render later/on top |
| **Visible** | Checkbox | Whether the entity renders and participates in normal updates |
| **Parallax X / Y** | Number | Camera-scroll multipliers (`0` fixed, `1` normal) |
| **Blur** | Number | Base blur radius in logical pixels |
| **Brightness** | Number | Exposure multiplier (`1` is neutral) |

All numeric and text fields are editable text fields. Press **Enter** or leave the field to commit a value. Changes are pushed through the `CommandStack` for undo/redo support (`Ctrl+Z` / `Ctrl+Y`).

The panel groups fields under headings such as **Transform**, **Sprite**, **Text**, **Physics Body**, **Emission**, and **Appearance**. Labels use a consistent width so fields remain aligned as the selected entity type changes.
...
- Each change creates a descriptive `FunctionalCommand` or `SetDoublePropertyCommand` capturing the old and new values
- **Ctrl+Z** undoes the last property change
- **Ctrl+Y** / **Ctrl+Shift+Z** redoes the last undone change
- Pressing Enter and then leaving a field does not duplicate the same change
- Re-entering the current value does not create a no-op undo entry
...
When nothing is selected, the Inspector keeps its header and help available while showing **"No entity selected"** and a concise next step. Select an entity on the canvas or in the entity tree to populate the panel.

## Validation and Feedback

- Numeric fields accept only finite Java numbers; `NaN`, infinities, blanks, and malformed values are rejected.
- Rejected values are restored to the current scene value instead of leaving stale invalid text in the field.
- A local status line confirms successful edits and explains invalid input; the same message is mirrored to the editor status bar.
- Text fields, number fields, and color/alignment controls have accessible labels or descriptions.
