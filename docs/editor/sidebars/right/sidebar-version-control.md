...
### Utility Icon Contact Sheet

![Version Control utility icon contact sheet](../../../assets/images/sidebars/version-control/version_control_icon_contact_sheet.png)

The production icon family uses dimensional Windows 7-style vector artwork. Refresh is supplied by
the shared `RefreshIcon`; the remaining Version Control roles are supplied by `VersionControlIcon`.
...
|--------|-----------|---------|-------------|
| **Refresh** | `RefreshIcon` | Refresh status and check online when needed | `git status`, `git branch`, `git remote`, periodic `git fetch --all --prune` |
| **Check Online** | `VersionControlIcon.CHECK_ONLINE` | Check whether the remote has incoming work | `git fetch --all --prune` |
| **Get Updates** | `VersionControlIcon.PULL` | Download incoming snapshots safely | `git pull --rebase --autostash` |
| **Send Online** | `VersionControlIcon.PUSH` | Upload saved snapshots to the remote | `git push`, or `git push -u origin <branch>` when no upstream is set |
