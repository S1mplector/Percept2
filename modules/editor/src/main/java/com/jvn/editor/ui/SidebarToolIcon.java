...
public final class SidebarToolIcon extends StackPane {
  ...
  private static Region artwork(Kind kind, String color, double size) {
    return switch (kind) {
      ...
      case APPLY -> CssIcon.check(color);
      case LINK -> SidebarToolArtwork.link(color);
      case HOME -> CssIcon.home(color);
      case COPY -> CssIcon.copy(color);
      case SELECT -> CssIcon.rectSelect(color);
      ...
      case PERSON -> CssIcon.person(color);
      case IMAGE -> CssIcon.landscape(color);
      case EDIT -> CssIcon.edit(color);
      ...
      case SPARKLES -> CssIcon.sparkles(color);
      case HELP -> CssIcon.help(color);
      case LABEL -> SidebarToolArtwork.label(color);
      case MOVIE -> CssIcon.movie(color);
      case LOCATE -> CssIcon.nearMe(color);
      case MEMORY -> SidebarToolArtwork.memory(color);
      case DOCK -> CssIcon.dock(color);
    };
  }
